...
- [ ] U7 — page-scan badge UX behind an explicit opt-in site-access grant. **Design drafted (2026-07-18) — see the U7 milestone section: per-site `optional_host_permissions` + `scripting`, browser-rendered consent on an explicit popup gesture, grant list owned by the browser's permission store, lazy visible-only scanning with a 2-verification concurrency cap, text-only tier pills in a shadow root (no icons — no checkmarks/shields ever), honest "not examined" markers for non-granted image hosts, U7a/b/c implementation split. Remaining: maintainer approval of the design as a dated Decision Log entry; implementation is blocked until then.**
...
- (Pending) **U7 design approval** — the maintainer's dated approval (or requested changes) of the U7 design goes here; implementation may not start before it. The permissions delta on the table: `optional_host_permissions: ["*://*/*"]` (granted per-site at runtime only) and `scripting` (to register content scripts scoped to grants). Nothing else in the design changes the trust surface.
