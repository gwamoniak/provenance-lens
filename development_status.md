...
| 2 — Watermark | **Real for one scheme (roadmap W1, 2026-07-31)**: `WatermarkDetector` trait + the Stable Diffusion invisible-watermark decoder (exact-payload dwtDct, ceiling `Indication`), CLI/native builds via the default-on `watermark-dwt` feature; the WASM/extension build opts out (size budget). Other schemes need a public spec + measured FPR (revised M5 gate). | Large-corpus FPR measurement (≥2000 clean images) before W2: `cargo run -p provenance-core --example measure_fpr -- <dir>`. |
...
4. **Roadmap plan (2026-07-31, maintainer-approved)**: `PROVENANCE_LENS_ROADMAP_EXECPLAN.md`, authored from an external technical review of 0.2.0 — **R1 done** (SynthID non-detectability caveat; EU AI Act Art. 50 positioning), **R2 done** (trust-list staleness defenses, merged with crypto sign-off), **W1 done** (the Stable Diffusion invisible-watermark detector — first real Layer-2 detector, first achievable Indicated verdict). Next: the W1→W2 FPR gate (measure on ≥2000 clean images), then W2 (IMATAG Stable Signature + calibration corpus); W3 SynthID stays vendor-gated; W4 registry-before-heuristics is the recorded ordering.
