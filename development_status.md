...
4. **Roadmap plan (new, 2026-07-31)**: `PROVENANCE_LENS_ROADMAP_EXECPLAN.md`, authored from an external technical review of 0.2.0 — R1 docs honesty (SynthID caveat, EU AI Act Art. 50 positioning), R2 trust-list staleness defenses (workflow failure alerting + dated "signer not on trust list" evidence sub-state), W1–W2 a staged real watermark layer (open SD DWT-DCT decoder, then IMATAG Stable Signature + calibration corpus, each behind measured-FPR gates), W3 SynthID vendor-gated, W4 registry-before-heuristics. **Maintainer decisions pending**: the M5 gate revision and the M6-priority call (both recorded as proposals in the wedge plan's Decision Log).
5. **Gated milestones** (wedge plan): M5 watermark / M6 registry / M7 heuristics — open only when their gates open (M5's gate revision is proposed, see above); `lens-research` tracks availability.
