...
4. **Roadmap plan (2026-07-31, maintainer-approved)**: `PROVENANCE_LENS_ROADMAP_EXECPLAN.md`, authored from an external technical review of 0.2.0 — **R1 done** (SynthID non-detectability caveat in the README diagram; EU AI Act Art. 50 positioning in README + manual), R2 next (workflow failure alerting + dated "signer not on trust list" evidence sub-state; crypto sign-off branch), W1–W2 a staged real watermark layer (open SD DWT-DCT decoder, then IMATAG Stable Signature + calibration corpus, each behind measured-FPR gates), W3 SynthID vendor-gated, W4 registry-before-heuristics (recorded ordering). The M5 gate revision and M6-priority proposals were **approved 2026-07-31** ("approved, start R1"), recorded in the wedge plan's Decision Log.
