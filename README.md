...
One asset flows through four layers, ordered by evidentiary strength; findings combine conservatively (tamper evidence outranks everything):
    ...
    bytes ──▶ 1. C2PA proof        cryptographic manifest validation   → can prove
              2. Watermark         open-spec decoders (SD dwtDct, …)   → can indicate
...
The watermark layer decodes openly specified schemes only — today the Stable Diffusion invisible watermark (exact payload match, never a similarity score; CLI and native builds, not yet in the extension engine). A hard caveat: **SynthID is not third-party-detectable.** Google publishes no spec, decoder weights, or public API for it, so only Google's own infrastructure can verify a SynthID watermark — no independent tool can, this one included, and no lookalike classifier will ever stand in for it here.
...
The Layer-1 wedge shipped and has since been hardened and extended (2026-07-18): real C2PA validation against the official conformance trust list (shipped as an updatable data file, refreshed by a monthly review PR), an eight-vector self-verifying test corpus (JPEG + PNG) exercised natively, through the compiled WASM artifact, and in real browsers, cert-policy pinning tests, a fuzz target, and CI on every push. The build order is deliberate: a **Layer-1-only** tool first — small but provable — because surfacing "Tampered / credentials stripped" at scale is the wedge that pressures platforms to stop stripping Content Credentials. The watermark layer became real on 2026-07-31 (roadmap W1): the Stable Diffusion invisible-watermark decoder, transcribed from the open reference implementation and tested against vectors that implementation generated. Detectors without a public spec, and the registry layer, stay gated; where a layer cannot run it honestly reports itself as not evaluated. The plans live in `PROVENANCE_LENS_EXECPLAN.md` (the wedge), `PROVENANCE_LENS_UPGRADES_EXECPLAN.md` (the executed upgrade wave), and `PROVENANCE_LENS_ROADMAP_EXECPLAN.md` (what's next).
