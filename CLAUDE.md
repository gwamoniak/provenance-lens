...
**M0–M2 closed** (M1 signed off by the maintainer per the human-review rule). Layer 1 is real (c2pa 0.89.2, default features off + `rust_native_crypto`; `lens verify --trust-anchors`; self-verifying vector corpus, regenerate: `cargo run -p provenance-core --example gen_vectors`). The WASM engine builds reproducibly (wasm-pack → system wasm-opt with flags from the wasm-packaging skill → `node scripts/wasm_smoke.mjs`), passes the corpus through the real artifact, and exports `verify_bytes(bytes, mediaType?, trustAnchorsPem?)`; size budget ≤ 7 MB raw / ≤ 2.5 MB gzipped (measured 6.41 / 2.15). 19/19 tests green. **M3 implemented** (context menu → fetch → engine → badge + popup; trust anchors as data in `extension/trust/anchors.pem`, empty placeholder until M4; smoke harness `scripts/serve_testpage.mjs`); the chrome.* glue awaits the human-run browser smoke scripted in the ExecPlan's M3 section — that closes M3, then M4 ships the wedge. M1 review follow-ups recorded: cargo-fuzz target, cert-policy tests, production trust-list distribution (M4). No GitHub remote yet (`gh` not installed). **Gated milestones:** M5 watermark (needs a runnable licensed vendor detector), M6 registry (needs a deployed transparency log), M7 heuristics (optional, may never ship) — do not start a gated milestone without its gate satisfied.
