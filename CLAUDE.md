...
**The wedge (M0–M4) shipped, the hardening pass is merged with cryptography sign-off, and the upgrades plan (`PROVENANCE_LENS_UPGRADES_EXECPLAN.md`, U1–U7) is fully executed** — U5 (Firefox) and U7 (opt-in page scanning) browser-smoked by the maintainer. Today's surface: `lens verify [--json] [--trust-anchors <PEM>] <FILE>...` with worst-verdict exit codes; Verified reports carry a credential summary (claim generator, signing time, declared digitalSourceType); eight-vector JPEG+PNG corpus tested hint-free at every level (regenerate: `cargo run -p provenance-core --example gen_vectors`); CI on every push (`.github/workflows/ci.yml`, plus weekly advisory audit/fuzz and the monthly trust-list refresh PR); one extension codebase for Chrome + Firefox ≥128; npm publish prep via `sh scripts/package_npm.sh`; page scanning behind per-site runtime grants with text-only verdict pills. 30/30 tests green; wording audit covers README, popup, skill, and the npm README. **0.2.0 is released (2026-07-18): submitted to the Chrome Web Store and Firefox AMO (in review) and published to npm.** The repo went public 2026-07-18. The c2pa SaltHash-underflow fix is already merged on c2pa-rs `main` (unreleased; the upstream report was NOT filed — it would duplicate resolved work); `.github/workflows/c2pa-bump-check.yml` (weekly) watches for the release carrying it and opens a bump PR (never merges — crypto sign-off surface). Open — maintainer actions: sign off that bump PR when it appears (then flip the CI fuzz job to enforcing and decide on the panic guard), and answer store reviewers (collateral in `docs/STORE_LISTING.md`). A third plan, `PROVENANCE_LENS_ROADMAP_EXECPLAN.md` (2026-07-31, from an external review of 0.2.0), stages the path forward and was maintainer-approved the same day, including its two proposals: R1 docs honesty + EU AI Act positioning (**done**), R2 trust-list staleness defenses (**done**, merged with crypto sign-off — the valid-but-unanchored detail now names the trust list's own date), W1 (**done** — Layer 2 is real for one scheme: the Stable Diffusion invisible-watermark decoder in `layers/sd_dwt.rs`, exact-payload match only, ceiling Indication, behind the default-on `watermark-dwt` feature that provenance-wasm opts out of; vectors from the verbatim reference embedder via `scripts/gen_watermark_vectors.py`), W2 (**done** — FPR gate passed 0/2,861, then the IMATAG Stable Signature bzh classifier in `layers/stable_signature.rs` behind the opt-in `stable-signature` feature via pure-Rust tract, model runtime-supplied with `--watermark-model <ONNX>` exported by `scripts/export_stable_signature_onnx.py`; calibration corpus + measured per-transformation TPR/FPR published in `docs/CALIBRATION.md` — zero false positives observed anywhere, DWT 0% TPR after any transform, bzh 30–85%), W3 SynthID vendor-gated, W4 (**done** 2026-08-02 — the M6 scoping plan `PROVENANCE_LENS_REGISTRY_EXECPLAN.md`: PDQ-256, RFC 6962 log with client-verified inclusion proofs, git-backed-pilot vs single-binary-service deployment shapes; awaiting the maintainer's G0 acceptance + shape choice). **Gated milestones (wedge plan):** M5 watermark (gate revised 2026-07-31; W1–W2 delivered two detectors + published calibration), M6 registry (scoping authored; gate opens on the maintainer's G0 entry in the registry plan; its G1–G4 stay individually gated, proof verifier under crypto sign-off), M7 heuristics (optional, may never ship) — do not start a gated milestone without its gate satisfied. Dev machine: Windows 11, `D:\sandbox\projects\provenance-lens` (rust toolchain, wasm-pack, nightly + cargo-fuzz all installed; binaryen/wasm-opt absent — release packaging tolerates it). Remote: https://github.com/gwamoniak/provenance-lens (**public** as of 2026-07-18; `origin`).
