...
**M0 and M1 closed** (M1 signed off by the maintainer per the human-review rule and merged). Layer 1 is real: c2pa 0.89.2 (default features off + `rust_native_crypto` — no OpenSSL, no HTTP clients compiled in), `lens verify --trust-anchors`, self-verifying vector corpus in `crates/provenance-core/tests/vectors/` (regenerate: `cargo run -p provenance-core --example gen_vectors`), 18/18 tests green, wasm32 type-check already passing. **M2 (WASM artifact + parity + size budget) is next.** M1 review follow-ups recorded: cargo-fuzz target, cert-policy tests, production trust-list distribution (M4). No GitHub remote yet (`gh` not installed). **Gated milestones:** M5 watermark (needs a runnable licensed vendor detector), M6 registry (needs a deployed transparency log), M7 heuristics (optional, may never ship) — do not start a gated milestone without its gate satisfied.
