...
- [ ] Post-wedge hardening backlog (from the M1 review, accepted as follow-ups, not blockers): cargo-fuzz target for manifest parsing (needs nightly toolchain), cert-policy pinning tests (expired certs, weak algorithms — currently inherited from the c2pa crate), automated trust-list refresh cadence.
