...
- [x] (2026-07-31) Third plan authored from an external technical review of 0.2.0: `PROVENANCE_LENS_ROADMAP_EXECPLAN.md` (R1 docs honesty/EU-AI-Act positioning, R2 trust-list staleness defenses, W1–W2 a staged real watermark layer with measured-FPR gates, W3 SynthID vendor-gated, W4 registry-before-heuristics). Both of its proposals — revising this plan's M5 gate (licensed-vendor → open-spec detector with measured FPR) and recording M6-before-M7 priority — were maintainer-approved 2026-07-31 ("approved, start R1"; dated Decision Log entry below); the M5 milestone text carries the revised gate.
- [x] (2026-08-02) M6 scoping document authored (roadmap W4): `PROVENANCE_LENS_REGISTRY_EXECPLAN.md` — the "explicit plan revision scoping standing one up" this plan's M6 gate names. The gate OPENS when the maintainer records acceptance (G0) in that plan's Decision Log; implementation milestones there stay individually gated, and its inclusion-proof verifier falls under the standing cryptography sign-off rule.
- [ ] Post-wedge (each gated, see Milestones): M5 watermark layer (two detectors delivered via roadmap W1–W2, calibration published), M6 registry layer (scoping authored, G0 pending), M7 heuristics layer (gated indefinitely; registry-before-heuristics ordering recorded 2026-07-31).
...
- Decision: **Roadmap proposals approved** — the maintainer (gwamoniak) approved `PROVENANCE_LENS_ROADMAP_EXECPLAN.md` including its two proposals: (a) the M5 gate is revised from "a runnable, licensed vendor detector" to "a runnable detector with a public specification or published weights, whose false-positive rate we measure ourselves on a clean corpus before it may contribute to verdicts" — which the fully open Stable Diffusion invisible-watermark DWT-DCT decoder satisfies today, so the roadmap plan's W1 may start when ordered; (b) registry-before-heuristics (M6 ahead of M7) is the recorded strategic ordering. The M5 milestone text below now carries the revised gate.
...
**M5 — Watermark layer (GATED — gate revised 2026-07-31 with maintainer approval, see Decision Log: a runnable detector with a public specification or published weights, whose false-positive rate we measure ourselves on a clean corpus before it may contribute to verdicts; originally "a runnable, licensed vendor detector"). Execution is staged in `PROVENANCE_LENS_ROADMAP_EXECPLAN.md` (W1–W2).** Statistical detection; ceiling `Indication`. Introduces the `WatermarkDetector` trait (vendor name + probe; the layer holds a pluggable detector list, per the watermark-detection skill) so vendors plug in without core changes; every detector's false-positive rate is measured on the clean corpus before it may contribute to verdicts. Do not start, and do not fake with a lookalike classifier, until the gate is satisfied; `lens-research` tracks when it opens.
